...
Pocketry.prototype.draggable = function() { // TODO: refactor excessive use of closures
  ...
  var onPick = Pocketry.debounce(50, function(draggie, ev, pointer) {
    self._dragTile = determineTile(draggie.position, self.layout.matrix,
      self.slotSize);
    self.publish('tile-picked', self._dragTile);
  });
...
Pocketry.prototype.onDrop = function(draggie, ev, pointer) {
	...
	if(Math.abs(draggie.dragPoint.x) + Math.abs(draggie.dragPoint.y) === 0) { // TODO: tolerance?
		return;
	}
	...
	if(t1) {
		if(t1.placeholder === 1) {
			...
			layout.walk(startPos, function(rowIndex, colIndex, span) {
				...
				var tile = this.matrix.get(rowIndex + span, colIndex);
		...
		var center = (t1.position.x + t1.size[0] / 2) * this.slotSize;
