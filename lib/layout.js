...
//    return placeholders || [];
  ...
  function nextStep(prevSelection, stepNumber, stepSize, scanArea) {
    ...
    var iterationsPerRow = maxRows - stepSize[1] + 1;

    // this function determines how many rows
    // were already covered by iterations
    function scannedRows(stepNumber, iterationsPerRow) {
      return Math.floor(stepNumber / iterationsPerRow);
    }

    // this function determines how many blocks
    // were already covered by iterations
    function scannedBlocks(stepNumber, iterationsPerRow) {
      return Math.floor(stepNumber / (iterationsPerRow * iterationsPerRow));
    }

    // this function determines how many columns were covered
    // to the left side of the current cursor
    function columnOffset(stepNumber, iterationsPerRow) {
      return iterationsPerRow * scannedBlocks(stepNumber, iterationsPerRow);
    }

    prevSelection.row.start =
      scannedRows(stepNumber, iterationsPerRow)
        % iterationsPerRow;

    prevSelection.col.start =
      columnOffset(stepNumber, iterationsPerRow)
        + stepNumber % iterationsPerRow;
