...
var i, j, tile;
var board = new Board(colCount, rowSpan);
...
// render
var container = $('.tiles').empty();
tiles = board.render(container, 100);
...
for(i = 0; i < board.matrix.length; i++) {
	...
	for(j = 0; j < board.matrix[i].length; j++) {
		...
		tile = board.matrix[i][j];
		if(tile && tile.id) {
			cell.text(tile.icon + ' ' + tile.id);
			cell.css('background-color', tile.color);
		}
	}
}
table.prependTo(document.body);
